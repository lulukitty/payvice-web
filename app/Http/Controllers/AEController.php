...
class AEController extends Controller
{
    ...
    /***
     * Abuja Electric Distribution Company 
    ...
    public function meterLookup(Request $request)
    {
        ...
        $validator =  Validator::make($request->all(), RequestRules::getRule('ABUJA_ELECTRIC_LOOKUP'));
        if ($validator->fails()) {
        ...
        \Log::info("AEDC validation Params". print_r($params, true));
        ...
        \Log::info("AEDC Validation Payment Params being sent to Vas 4.0: ". print_r($parameters, true));
        ...
        try {
            ...
            \Log::info("AEDC Validation Direct Response ". print_r($responseValidationData, true));
            ...
            if($responseValidationData['responseCode'] == "00"){
                ...
                if (isset($request->view)) {
                    $status = true;
                    $data = $this->mailTemplate('aedc_payment', $responseValidationData['data']);
            ...
            if ($e->hasResponse()) {
                ...
                \Log::info("AEDC Validation Exception ". print_r($errorMsg, true));
                return $errorMsg;
            }
    ...
    public function payUtility(Request $request)
    {
        ...
        try {
            $response = $this->guzzleClient()->request('POST', HttpRequests::baseURI() . '/vas/abuja/payment', [
                'json' => $jsonParams,
            ...
            if ($transactionStatus == true) {
                ...
                if (isset($request->view)) {
                    $msg = "Transaction Failed. " . $responseData['description'] . " Please Try Again";
                    $status = false;
                    $data = $msg;
                    return $this->returnOutput($status, $data);
                } else {
