...
class PHEController extends Controller
{
    protected $helper;
    protected $client;
    ...
    /***
     * PortHarcourt Electric Distribution Company 
    ...
    public function meterLookup(Request $request){
        ...
        $validator =  Validator::make($request->all(), RequestRules::getRule('PH_ELECTRIC_LOOKUP'));
        if($validator->fails()) {
        ...
        \Log::info("Phedc validation Params". print_r($params, true));
        ...
        \Log::info("PHEDC Validation Payment Params being sent to Vas 4.0: ". print_r($parameters, true));
        ...
        try {
            ...
            \Log::info("PHEDC Validation Direct Response ". print_r($responseValidationData, true));
            ...
            if($responseValidationData['responseCode'] == "00"){
                ...
                if(isset($request->view)){
                    $status = true;
                    $data = $this->mailTemplate('phed_payment',$responseValidationData['data']);
            ...
            if ($e->hasResponse()) {
                if(isset($request->view)){
                    ...
                    \Log::info("Phedc Validation Exception ". print_r($errorMsg, true));
                    return $this->returnOutput($status,$data);
                } else {
                    $errorMsg = $this->jsonToArray($e->getResponse()->getBody()->getContents());
                    \Log::info("Phedc Validation Exception ". print_r($errorMsg, true));
                    return $this->displayValidationError($errorMsg['message']);
                }
            }
        } 
    ...
    public function payUtility(Request $request){
        ...
        if(isset($params['amount'])){
            $amount = (int) $params['amount'] * 100; // In Kobo
        }

        if(strtolower($params['type']) == "prepaid"){
            $type = "prepaid";
        } else {
            $type = "postpaid";
        }
        ...
        try {
            $response = $this->guzzleClient()->request('POST', HttpRequests::baseURI().'/vas/phed/payment', [
                'json' => $jsonParams,
            ...
            if($transactionStatus == true){
                if(isset($request->view)){
                    if($type == "postpaid"){
                        ...
                        $msg = "Status: ". $responseData['message']. "; PayRef: ". $responseData['reference']. "; Amount Paid: &#8358;". $responseData['amount'] ."; TOKEN: ".$responseData['token'];
                    }
                    $status = true;
                    $data = $msg;
                    return $this->returnOutput($status,$data);
