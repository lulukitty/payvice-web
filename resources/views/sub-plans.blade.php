...
<div id="planForm">
  <form action="#" method="POST" id="planSelectForm">
    <ul class="list-group shadow">
      ...
      <li class="list-group-item">
        <label class="radio labelchk no-margin no-bg">
          <input type="radio" class="rel-radioright" name="payMethodPlan" data-value="{{ $plan['amount'] }}" value="{{ $plan['product_code'] }}">
          <spam class="imgchkk">
            <img src="{{ URL::asset('assets/img/payvice_logob.png') }}">
          </spam>   
          <span>{{ $plan['name'].' - N'.$plan['amount'] }}</span>
        </label>
      </li>
      @endforeach
    ...
    <div class="form-group">
      ...
      <button id="planSelectFormButton" type="submit" class="btn-customb btn-block arrow-btn waves-effect waves-classic proceedMC">
...
$('#showmenu').on('click', function(event) {
  ...
  if ($('input[name=payMethod]').val().length === null) {
    ...
    $.ajax({
      ...
      success: function(result){
        if (result['status'] === 1) {
          swal('Cable TV subscribtion', result['message'], 'success');
        }else{
          swal('Cable TV subscribtion', result['message'], 'error');
        };
